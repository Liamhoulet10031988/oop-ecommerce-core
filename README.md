...
- вывод товаров категории через `@property`;
- создание товара из словаря через `Product.new_product()`;
- приватное хранение цены товара;
- проверка новой цены через `setter`.

- строковое отображение товаров через __str__;
- строковое отображение категорий через __str__;
- расчет полной стоимости двух товаров через __add__;
- перебор товаров категории через итератор CategoryIterator.
